...
# Import data retrieval functions from other modules
try:
...
def get_health_status(value, threshold_dict, higher_is_better=True):
    ...
    Returns: status ('CRITICAL', 'WARNING', or 'HEALTHY')
    """
    if higher_is_better:
...
def generate_health_report(cluster_id, efficiency_data, status_data, runtime_data, held_data):
    """Generate health report as a list of findings"""
    findings = []
    ...
    if held_count > 0:
        ...
        if held_status == "CRITICAL":
            reason = f"{held_count} jobs held ({held_pct:.1f}%), {unique_reasons} unique reasons"
        elif held_status == "WARNING":
            reason = f"{held_count} jobs held ({held_pct:.1f}%), {unique_reasons} unique reasons"
        else:
            reason = f"{held_count} jobs held ({held_pct:.1f}%)"
    ...
    if cv > 0:
        ...
        findings.append({
            "aspect": "Runtime Consistency",
            "status": cv_status,
            "value": f"CV={cv:.2f}",
            "jobs": total_runtime_jobs,
            "reason": reason,
            "tool": f"python histogram.py {cluster_id}"
        })
    else:
    ...
    if total_jobs > 0:
        completed_pct = (completed / total_jobs) * 100
        status_str = f"{completed} completed, {running} running, {idle} idle"
...
def print_health_summary(cluster_id, findings):
    """Print concise health summary table"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python cluster_health.py <ClusterId>")
        print("\nDescription:")
        print("  Provides concise cluster health summary with tool recommendations")
        print("\nExample:")
        print("  python cluster_health.py 12345")
        print("\nNote:")
        print("  Make sure to run 'python fetch_cluster_data.py <ClusterId>' first")
        sys.exit(1)
    
    cluster_id = sys.argv[1]
    
    # Gather data from all analysis modules (quietly)
    efficiency_data = get_analytics_data(cluster_id)
    if not efficiency_data:
        print(f"{color_text('ERROR', Colors.RED)}: Could not load analytics data")
        print(f"Please run: python fetch_cluster_data.py {cluster_id}")
        sys.exit(1)
    
    status_data = get_dashboard_data(cluster_id)
    if not status_data:
        status_data = {"total_jobs": efficiency_data.get("total_jobs", 0), "status_counts": {}, 
                      "held": 0, "held_pct": 0, "completed": 0, "running": 0, "idle": 0}
    
    runtime_data = get_histogram_data(cluster_id)
    if not runtime_data:
        runtime_data = {"cv": 0, "fast_jobs": 0, "fast_jobs_pct": 0, "total_runtime_jobs": 0, "correlation": 0}
    ...
    findings = generate_health_report(cluster_id, efficiency_data, status_data, runtime_data, held_data)
